...
["btnRight", "ArrowRight"],
  ...
  function bindHold(id, code){
    ...
    const down = (e)=>{
      ...
      if(e.pointerType === "touch") el.style.touchAction = "none";
...
// ===== Bottom buttons wiring (Music / Start / Reset) =====
const startBtnEl = document.getElementById("startBtn");
const resetBtnEl = document.getElementById("resetBtn");
const musicBtnEl = document.getElementById("musicBtn");

// Start: اگر Intro هست شروع کن، وگرنه ریست کن
...
// Reset: همیشه ریست
resetBtnEl?.addEventListener("click", ()=> resetAll(true));

// Music: Toggle روشن/خاموش
musicBtnEl?.addEventListener("click", ()=> toggleMusic());
// ===== Force mobile click support =====
["startBtn","resetBtn","musicBtn"].forEach(id=>{
  const el = document.getElementById(id);
  if(!el) return;
...
// ===== Robust mobile/desktop button bindings (fix Start/Reset/Music) =====
(function bindUIButtons(){
  ...
  // Start: هم دکمه پایین (startBtn) هم دکمه موبایل (btnStart)
  bindPress("startBtn", ()=>{
    if(state === STATE.INTRO || state === STATE.LOSE || state === STATE.WIN) startGame();
    else resetAll(true);
  });
  bindPress("btnStart", ()=>{
    if(state === STATE.INTRO || state === STATE.LOSE || state === STATE.WIN) startGame();
  ...
  // Reset: دکمه موبایل (btnReset). اگر دکمه پایینی برای reset ساختی و id دادی resetBtn، این هم ساپورت میشه.
  bindPress("btnReset", ()=> resetAll(true));
  bindPress("resetBtn", ()=> resetAll(true));

  // Music: هم دکمه پایین (musicBtn) هم اگر دکمه موبایل جدا داشتی
  bindPress("musicBtn", ()=> toggleMusic());
  bindPress("btnMusic", ()=> toggleMusic());
})();